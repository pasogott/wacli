...
The normal `pnpm build`, GoReleaser checks, snapshot builds, and Linux/Windows release builds remain credential-free. Official builders retain Go's top-level `-trimpath`; Go 1.26.5 intentionally omits the informational `-ldflags` buildinfo field in that mode, so verification reads the linked runtime variables themselves. The protected cross-build workflow runs the real GoReleaser release twice with separate fresh HOME, module, build, and temp caches, then requires byte-identical binaries, archives, and checksums before exposing the first output set.
